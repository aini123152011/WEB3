...
- **特点**：
  ...
  - 隔离执行环境
  - Gas机制控制资源消耗
- **EVM兼容链**：BSC、Polygon、Avalanche等

### 账户类型

```
外部账户 (EOA)                    合约账户 (Contract Account)
├─ 由私钥控制                     ├─ 由代码控制
├─ 可以发起交易                   ├─ 被动响应交易
├─ 无代码存储                     ├─ 存储智能合约代码
└─ 示例：MetaMask钱包            └─ 示例：Uniswap合约
```
...
- **示例**：
  ...
  Gas Used: 21000
  Gas Price: 50 Gwei
  总费用 = 21000 × 50 = 1,050,000 Gwei = 0.00105 ETH
  ```
...
// 整数型
uint256 public unsignedInt = 123;  // 无符号整数
int256 public signedInt = -123;     // 有符号整数
...
智能合约是WEB3的核心，Solidity是最流行的智能合约语言。掌握基本语法、常见模式和安全实践是成为优秀区块链开发者的基础。

**下一章预告**：我们将学习如何使用Web3.js和Ethers.js与智能合约交互 🔌

---

> 💡 **实践建议**：在Remix上编写和部署这些示例合约，在测试网（Sepolia或Goerli）上实验。
