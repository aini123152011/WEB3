# WEB3测试工程师完全指南 - 安全测试与工具篇 🛡️

## 第一章：安全测试基础

### 1.1 WEB3安全测试特殊性

**与传统安全测试的差异**：
```
传统Web安全          WEB3安全
├─ SQL注入           ├─ 重入攻击
├─ XSS攻击           ├─ 整数溢出/下溢
├─ CSRF              ├─ 访问控制漏洞
├─ 权限绕过          ├─ 前端运行(MEV)
└─ 数据泄露          └─ 私钥泄露

风险：数据损坏       风险：资金永久损失
可修复：是           可修复：通常不可以
```

### 1.2 OWASP Smart Contract Top 10（必记）

1. **SC01: 重入攻击 (Reentrancy)**
2. **SC02: 访问控制 (Access Control)**
3. **SC03: 算术问题 (Arithmetic Issues)**
4. **SC04: 未检查的外部调用 (Unchecked Calls)**
5. **SC05: 拒绝服务 (DoS)**
6. **SC06: 糟糕的随机性 (Bad Randomness)**
7. **SC07: 前端运行 (Front-Running)**
8. **SC08: 时间操纵 (Time Manipulation)**
9. **SC09: 短地址攻击 (Short Address Attack)**
10. **SC10: 未知未知 (Unknown Unknowns)**

## 第二章：智能合约安全测试实战

### 2.1 重入攻击测试

**漏洞代码**：
```solidity
// ❌ 不安全
contract Vulnerable {
    mapping(address => uint) public balances;
    
    function withdraw() public {
        uint bal = balances[msg.sender];
        (bool sent,) = msg.sender.call{value: bal}("");
        require(sent);
        balances[msg.sender] = 0; // 状态变更在后
    }
}
```

**测试用例**：
```solidity
// test/Reentrancy.t.sol
contract ReentrancyTest {
    function test_reentrancy_exploit() public {
        Attacker attacker = new Attacker(address(vulnerable));
        attacker.attack{value: 1 ether}();
        assertGt(address(attacker).balance, 1 ether); // 攻击成功
    }
}

contract Attacker {
    Vulnerable target;
    constructor(address _target) { target = Vulnerable(_target); }
    
    function attack() external payable {
        target.deposit{value: msg.value}();
        target.withdraw();
    }
    
    receive() external payable {
        if (address(target).balance >= 1 ether) {
            target.withdraw(); // 重入
        }
    }
}
```

**修复验证**：
```solidity
// ✅ 安全：CEI模式（Checks-Effects-Interactions）
function withdraw() public {
    uint bal = balances[msg.sender];
    balances[msg.sender] = 0; // 先更新状态
    (bool sent,) = msg.sender.call{value: bal}("");
    require(sent);
}
```

### 2.2 访问控制测试

**测试矩阵**：
```typescript
describe("Access Control", () => {
  it("only owner can pause", async () => {
    await expect(contract.connect(attacker).pause())
      .to.be.revertedWith("Ownable: caller is not the owner");
  });
  
  it("minter role required for mint", async () => {
    await expect(contract.connect(user).mint(user.address, 100))
      .to.be.revertedWith("MINTER_ROLE required");
  });

  it("admin can grant roles", async () => {
    await contract.grantRole(MINTER_ROLE, user.address);
    await expect(contract.connect(user).mint(user.address, 100))
      .to.not.be.reverted;
  });
});
```

### 2.3 整数溢出测试（Solidity <0.8）

**测试方法**：
```typescript
it("should not overflow", async () => {
  // 对于Solidity 0.8+，自动回滚
  await expect(contract.increment(ethers.MaxUint256))
    .to.be.revertedWithPanic(0x11); // Arithmetic overflow
});
```

### 2.4 时间依赖测试

**漏洞场景**：依赖block.timestamp做关键判断
```typescript
it("should not manipulate time", async () => {
  const currentTime = await time.latest();
  await time.increaseTo(currentTime + 86400); // +1天
  
  // 验证时间锁是否按预期工作
  await expect(contract.withdraw()).to.not.be.reverted;
});
```

### 2.5 前端运行(Front-Running)测试

**场景**：价格预言机更新、大额交易、拍卖出价
```typescript
describe("MEV Protection", () => {
  it("should use commit-reveal", async () => {
    const commit = ethers.keccak256(ethers.toUtf8Bytes("bid:1000:salt123"));
    await auction.commitBid(commit); // 提交承诺
    await time.increase(3600);
    await auction.revealBid("bid:1000:salt123"); // 揭示
  });
});
```

## 第三章：静态分析工具

### 3.1 Slither（Python）

**安装与使用**：
```bash
pip install slither-analyzer
slither contracts/ --print human-summary
slither contracts/ --detect reentrancy-eth,uninitialized-state
```

**常见检测项**：
- reentrancy-eth / reentrancy-no-eth
- uninitialized-state / uninitialized-storage
- arbitrary-send-eth / suicidal
- tx-origin / timestamp

**CI集成**：
```yaml
- name: Slither
  run: |
    pip install slither-analyzer
    slither . --fail-high --json-types detectors
```

### 3.2 Mythril（符号执行）

```bash
pip install mythril
myth analyze contracts/Token.sol
```

### 3.3 Solhint（代码规范）

```bash
npm i -D solhint
npx solhint 'contracts/**/*.sol'
```

**配置示例**（.solhint.json）：
```json
{
  "extends": "solhint:recommended",
  "rules": {
    "no-unused-vars": "error",
    "func-visibility": ["error", { "ignoreConstructors": true }],
    "state-visibility": "error",
    "max-line-length": ["warn", 120]
  }
}
```

### 3.4 4naly3er（多合一）

```bash
npm i -g 4naly3er
4naly3er contracts/
```

## 第四章：动态分析与模糊测试

### 4.1 Echidna（属性测试）

**安装**：
```bash
docker pull trailofbits/eth-security-toolbox
# 或编译安装：https://github.com/crytic/echidna
```

**属性函数示例**：
```solidity
contract TestERC20 is ERC20 {
    function echidna_totalSupply_constant() public view returns (bool) {
        return totalSupply() == 1_000_000e18;
    }
    
    function echidna_balance_geq_zero() public view returns (bool) {
        return balanceOf(msg.sender) >= 0;
    }
}
```

**运行**：
```bash
echidna-test contracts/TestERC20.sol --contract TestERC20
```

### 4.2 Foundry Fuzz（内置）

```solidity
function testFuzz_transfer(address to, uint256 amount) public {
    vm.assume(to != address(0) && amount <= 1e24);
    token.transfer(to, amount);
    assertEq(token.balanceOf(to), amount);
}
```

**运行**：
```bash
forge test --fuzz-runs 10000
```

### 4.3 Manticore（符号执行）

```python
from manticore.ethereum import ManticoreEVM

m = ManticoreEVM()
with open('Token.sol') as f:
    m.solidity_create_contract(f.read())

# 执行符号交易
m.transaction(caller=m.make_symbolic_address(), 
              address=contract, 
              data=m.make_symbolic_buffer(4+32+32))
```

## 第五章：审计工具与流程

### 5.1 MythX（商业平台）

```bash
npm install -g mythxjs-cli
mythxjs analyze contracts/Token.sol
```

### 5.2 Certora（形式化验证）

**规范示例**（.spec文件）：
```
methods {
    function totalSupply() external returns (uint256) envfree;
    function balanceOf(address) external returns (uint256) envfree;
}

invariant totalSupplyEqualsSumOfBalances()
    totalSupply() == sumOfBalances()
```

### 5.3 审计清单（Checklist）

**部署前必查**：
- [ ] 所有external/public函数有访问控制
- [ ] 无selfdestruct或谨慎使用
- [ ] 外部调用检查返回值
- [ ] 使用最新稳定Solidity（0.8+）
- [ ] 所有状态变更遵循CEI模式
- [ ] 敏感操作有事件记录
- [ ] 测试覆盖率>90%
- [ ] Slither无高危/中危警告
- [ ] 至少一次外部审计（主网）

## 第六章：测试工具生态

### 6.1 本地开发节点

| 工具 | 特点 | 使用场景 |
|------|------|---------|
| **Hardhat Node** | 集成方便，TypeScript友好 | 日常开发 |
| **Ganache** | 可视化界面，易用 | 学习阶段 |
| **Anvil (Foundry)** | 速度快，原生分叉 | 性能测试 |

```bash
# Hardhat
npx hardhat node

# Anvil
anvil --fork-url $MAINNET_RPC
```

### 6.2 测试网水龙头自动化

```typescript
// scripts/faucet.ts
import axios from 'axios';

async function getFaucet(address: string) {
  const response = await axios.post('https://sepolia-faucet.com/api/claim', {
    address,
    token: process.env.FAUCET_TOKEN
  });
  console.log(`Claimed: ${response.data.txHash}`);
}
```

### 6.3 Gas分析工具

```bash
# Hardhat Gas Reporter
npm i -D hardhat-gas-reporter
REPORT_GAS=true npx hardhat test

# Foundry Gas Snapshots
forge snapshot
```

### 6.4 RPC服务商对比

| 服务商 | 免费额度 | 主网Archive | 速度 |
|--------|---------|------------|------|
| **Alchemy** | 300M CU/月 | ✅ | 快 |
| **Infura** | 100K req/天 | ❌ | 中 |
| **QuickNode** | 按需付费 | ✅ | 很快 |
| **Ankr** | 有限免费 | ✅ | 中 |

## 第七章：安全最佳实践

### 7.1 开发阶段

```solidity
// ✅ 好的模式
contract Safe {
    // 1. 使用OpenZeppelin库
    using SafeERC20 for IERC20;
    
    // 2. 明确可见性
    uint256 private _value;
    
    // 3. 事件记录
    event ValueChanged(uint256 oldValue, uint256 newValue);
    
    // 4. CEI模式
    function withdraw(uint256 amount) external {
        require(balances[msg.sender] >= amount, "Insufficient");
        balances[msg.sender] -= amount; // Effect
        payable(msg.sender).transfer(amount); // Interaction
        emit Withdrawn(msg.sender, amount);
    }
    
    // 5. 使用modifier
    modifier onlyOwner() {
        require(msg.sender == owner, "Not owner");
        _;
    }
}
```

### 7.2 测试阶段

**必须覆盖的场景**：
```
✅ 正常流程
✅ 边界条件（0, max, max+1）
✅ 失败场景（余额不足、权限不足）
✅ 重入尝试
✅ 时间相关（过期、未生效）
✅ 并发交易（nonce、竞态）
✅ 恶意输入（超长数组、零地址）
```

### 7.3 部署阶段

**部署前检查**：
```bash
# 1. 编译优化
forge build --optimize --optimizer-runs 200

# 2. 字节码验证
sha256sum out/Contract.sol/Contract.json

# 3. 构造参数验证
cast abi-encode "constructor(string,uint)" "Token" 1000000

# 4. Gas估算
cast estimate --from $DEPLOYER $DATA
```

## 第八章：应急响应

### 8.1 暂停机制

```solidity
import "@openzeppelin/contracts/security/Pausable.sol";

contract SafeToken is ERC20, Pausable, Ownable {
    function transfer(...) public whenNotPaused override {
        super.transfer(to, amount);
    }
    
    function pause() external onlyOwner { _pause(); }
    function unpause() external onlyOwner { _unpause(); }
}
```

### 8.2 监控与告警

```typescript
// 监控异常事件
contract.on("EmergencyWithdraw", (user, amount, event) => {
  alert(`CRITICAL: EmergencyWithdraw ${amount} by ${user}`);
});

// 监控revert率
const tx = await contract.someFunction();
if (!tx.status) {
  metrics.increment('contract.reverts');
}
```

### 8.3 事件响应流程

```
发现异常
    ↓
立即暂停合约（如有Pause）
    ↓
通知核心团队
    ↓
分析根因（链上数据、交易回放）
    ↓
评估影响范围
    ↓
制定修复方案
    ↓
社区公告
    ↓
执行修复（如可升级）
    ↓
复盘与改进
```

---

## 📚 本章总结

WEB3安全测试是一个持续迭代的过程：

1. **工具为辅，理解为主**：工具会误报，需人工判断
2. **攻击者视角**：站在黑客角度思考
3. **纵深防御**：代码审查、自动化扫描、外部审计多层保障
4. **持续学习**：关注安全事件，学习新攻击手法

**下一章预告**：实战案例分析与面试指南 🎯

---

> 💡 **安全格言**：在WEB3，安全不是功能，而是生命线。每一行代码都可能关系到数百万美元。
